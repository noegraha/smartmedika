...
const PerencanaanPulangPasien = () => {
  ...
  //Observation - Kriteria Pasien untuk Rencana Pemulangan
  const [clinicalTerm, setclinicalTerm] = useState({});
  const [deskEncounter, setdeskEncounter] = useState("");
  const [clinicalTermAkhir, setclinicalTermAkhir] = useState({});

  const [modalTTvrs, setmodalTTvrs] = useState(false);
  //state tampil data
  const [listRencanaPulangOb, setlistRencanaPulangOb] = useState([]);
  const [listCarplan, setlistCarplan] = useState([]);
      ...
      coding: [
        {
          ...
          code: clinicalTerm.code,
          display: clinicalTerm.display,
        },
      ],
    },
    subject: {
      ...
      coding: [
        {
          ...
          code: clinicalTermAkhir.code,
          display: clinicalTermAkhir.display,
        },
      ],
    },
  };

  const dataCareplanpulang = {
  ...
  const klikRefresh = async () => {
    ...
    try {
      ...
      setlistRencanaPulangOb(results[0]);
      setlistCarplan(results[1]);
    ...
    <div>
      <Spin spinning={spCvg} tip="Loading... 😁">
          ...
          <Col span={12}>
            <Button
              ...
              onClick={() => {
                ...
                setclinicalTermAkhir({
                  code: "OV000072",
                  display: "Pasien dengan perawatan berkelanjutan atau panjang",
                });
                setdeskEncounter(
                  "Pemeriksaan Kriteria untuk Rencana Pemulangan" +
                  { ihsPasien } +
                  "di hari Kamis, 25 Desember 2022"
                );
              }}
              icon={<PlusOutlined />}
            >
        ...
        <Row>
          ...
          <Table
            ...
            dataSource={listRencanaPulangOb}
            pagination={false}
            size="small"
          />
        </Row>
        <Row>
          ...
          >
            Care Plan Rencana Pulang
          </Divider>
          <Table
            bordered
            loading={spCvg}
            columns={colRiwPenyakit}
            dataSource={listCarplan}
            pagination={false}
            size="small"
          />
        </Row>
      ...
      >
        ...
        <Button
          ...
          onClick={async () => {
            let rencanapulang = {
              data: dataRencanapulang,
              type: "Observation",
              id: "100",
            };
            let careplan = {
              data: dataCareplanpulang,
              type: "CarePlan",
              id: "101",
            };

            const codeGroups = [rencanapulang, careplan];

            try {
              // Jalankan semua permintaan data secara paralel
              await Promise.all(
                codeGroups.map((codeGroup) =>
                  postResource(codeGroup.data, codeGroup.type, codeGroup.id)
                )
              );
...
export default PerencanaanPulangPasien;
