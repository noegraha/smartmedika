...
const FormOrderResep = () => {
  ...
  let fLenz = barang.length;

  for (let i = 1; i < fLenz; i++) {
    barang.map((v) => ({ ...v, key: i }));
  }
  useEffect(() => {
    getAturanPakai();
  }, []);
  ...
  const [modalobatjadi, setModalObatJadi] = useState(false);
  const onAmbilObatJadi = (a, b, c) => {
    if (b === true) {
      setItems([
        ...items,
        {
          isRacikan: false,
          barangId: a.KodeBarang,
          quantity: jumlah,
      ...
      setItems(items.filter((item) => item.barangId !== a.KodeBarang));
    }
  };
  ...
  const [dosisracik, setDosisRacik] = useState(0);
  ...
  const [jenisracikid, setJenisRacikId] = useState("1");
  const [aturanpakaiidracik, setAturanPakaiIdRacik] = useState("1");
  ...
  return (
    <div>
          ...
          >
            <Form {...formItemLayout2}>
              <Row gutter={[8, 8]}>
                  ...
                  <Row>
                    <Col style={{ textAlign: "right" }} span={24}>
                      <Space>
                        ...
                        <Button type="primary" onClick={onTambahRacik}>
                          Tambah Racikan
                        </Button>
                      </Space>
                    </Col>
                ...
                {tampilracikan ? (
                  <Col span={12}>
                    <Form.Item label="Obat" style={{ marginBottom: 0 }}>
                      <Select
                        onFocus={() =>
                          unitorder === ""
                            ? message.warning("Pilih apotik terlebih dahulu!")
                    ...
                    <Row>
                      <Col style={{ textAlign: "right" }} span={24}>
                        <Button
                          htmlType="submit"
                          type="primary"
                          onClick={() => onTambahItemRacikan(noracikaktif)}
                        >
                          Ambil
                        </Button>
                      </Col>
      ...
      {/* Modal Obat Jadi */}
      <Modal
        visible={modalobatjadi}
        onCancel={(e) => setModalObatJadi(false)}
        title={
          <div
            style={{
              width: "100%",
      ...
      >
        <Table
          ...
          rowSelection={{
            ...
            onSelect: (record, selected, selectedRows) =>
              onAmbilObatJadi(record, selected, selectedRows),
          }}
          locale={{
            emptyText: (
              <Empty
        ...
        >
          <Column
            ...
            className="bgcolor"
          />
          <Column title="Nama Barang" key="desk" dataIndex="NamaBarang" />
          <Column title="Stock Unit" key="stunit" dataIndex="StockUnit" />
          <Column title="Stock Gudang" key="stgudang" dataIndex="StockGudang" />
        </Table>
      </Modal>
